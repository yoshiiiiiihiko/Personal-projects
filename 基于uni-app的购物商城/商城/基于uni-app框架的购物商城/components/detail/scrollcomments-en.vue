<template>
	<view class="p-3">
		<scroll-view scroll-x class="scroll-row">
			<view class="scroll-row-item rounded bg-light-secondary mr-2 p-2"
			style="width: 620rpx;height: 150rpx;"
			v-for="(item,index) in comments" :key="index">
				<view class="d-flex a-center mb-2">
					<image :src="item.userpic" mode="widthFix"
					style="width: 70rpx; height: 70rpx;"
					class="rounded-circle"></image>
					<view class="ml-2">
						<text class="font-md line-h">{{item.username}}</text>
						<text class="d-block font text text-light-muted line-h">{{item.create_time}}</text>
					</view>
					<view class="iconfont icon-dianzan text-light-muted font pl-1 ml-auto">
						{{item.goods_num}}
					</view>
				</view>
				<view style="width: 610rpx; overflow: hidden;">
					<text class="d-block font-md mb-3">
						{{item.context}}
					</text>
				</view>
				<view class="row">
					<view class="span24-8 px-1"
					v-for="(img,i) in item.imglist" :key="i">
						<image :src="img" mode="widthFix" style="height: 115rpx;" class="rounded"></image>
					</view>
				</view>
			</view>
		</scroll-view>
		<view class="d-flex j-center py-2 mt-2 text-primary a-center" @click="openDetailComment()">
				更多评论<view class="iconfont icon-you"></view>
		</view>
	</view>
</template>

<script>
	export default{
		props:['comments','goods_id'],
		methods:{
			openDetailComment(){
				uni.navigateTo({
					url:'../../pages/detail-comment/detail-comment?id='+this.goods_id
				})
			}
		}
	}
</script>

<style>
</style>
